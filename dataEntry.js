...
function employeeDataManagement(){
	let employeeData=fetchEmployee();

if(selectedRow==null){
	newDataInserted(emploeeData);
}
else{
...
function newDataInserted(emploeeData){
	let tbody=document.getElementById("table-body");
	let newRow = tbody.inserRow(0);
...
function updateRow(employeeData){
	...
	selectedRow.cell[2].innerHTML=employeeDesignation.value;
	selectedRow.cell[3].innerHTML=employeeDepartment.value;
	selectedRow.cell[4].innerHTML=employeeSalary.value;
	selectedRow.cell[5].innerHTML=employeeAddress.value;
